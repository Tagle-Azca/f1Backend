...
/**
 * Build the $set payload to upsert race results into MongoDB.
 * @param {object} jr  — a Jolpica race object with Results array
 */
...
/**
 * Fetch all race results for a year from Jolpica and upsert into MongoDB.
 * Retries individually any rounds that were skipped (no results in bulk) but have past dates.
 * Also syncs sprint results for sprint weekends.
 *
 * Throws on unrecoverable errors; partial failures are logged but not thrown.
 *
 * @param {string} year
...
export async function syncRacesForYear(year) {
  ...
  // Retry rounds that bulk returned empty — individual endpoint often has them sooner
    ...
    for (const jr of skippedRounds) {
      try {
        const r2 = await fetch(
          `${JOLPICA}/${year}/${jr.round}/results.json`,
          { headers: HEADERS, signal: AbortSignal.timeout(8_000) }
        )
        if (!r2.ok) continue
        const j2   = await r2.json()
        const race = j2?.MRData?.RaceTable?.Races?.[0]
  ...
  // Sync sprint results for sprint weekends
...
/**
 * Seed OpenF1 telemetry data for a year's races into Cassandra.
 * Fetches race sessions from OpenF1, then inserts lap times, positions,
 * pit stops, and stints per driver into the configured Cassandra keyspace.
 *
 * Returns a status string describing what was done.
 *
 * @param {string}  year
 * @param {number}  racesLimit  — maximum number of past sessions to process
 * @param {boolean} force       — if true, re-seed sessions even if already present
 * @returns {string}
 */
export async function seedRaceToCassandra(year, racesLimit, force) {
  const cassandra = getCassandraClient()
  if (!cassandra) throw new Error('Cassandra not connected')
